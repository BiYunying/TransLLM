    ...
    for idx, instruct_item in tqdm(enumerate(prompt_file)):
        ...
        with torch.inference_mode():
            ...
            if start_inx.shape[0] == 3 and end_inx.shape[0] == 3 and end_inx[2]-start_inx[2]==12:
                ...
                st_pre_out1 = model.relu(model.st_pred_linear_10(st_pre_embs1))


                st_pre_embs2 = hidden_states[:,
                            start_inx[2]+1:end_inx[2],
                            :].reshape(batch_size, -1, feature_nums, model.config.hidden_size)
                st_pre_out2 = model.relu(model.st_pred_linear_12(st_pre_embs2))

                st_pre_final = model.st_pred_linear_11(torch.cat([st_pre_out1, st_pre_out2], dim=-1)).reshape(st_pre_out1.shape[0],12,1,1)
...
if __name__ == "__main__":
    ...
    output_model='./checkpoints/transllm_5dataset_pems08/full_model'
    ...
    # eval_model(args, datapath, start_id, end_id)
